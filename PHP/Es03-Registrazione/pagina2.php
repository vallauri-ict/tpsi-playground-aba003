	...
	<body>
		...
		<?php
			...
			if(isset($_REQUEST["txtNome"]))
				...
				//richiede quello che è selezionato
			...
			if(isset($_REQUEST["chkHobbies"]))
			{
				...
				$hobbies = implode(',',$hobbies); //converte a stringa gli elementi dell array  eli divide con unja comma
			...
			$con = _connection("db_esercizio_3");
			...
			if($ris)
			{
				...
				echo("<p>$segni;</p>");
				echo("<p>$scoperta;</p>");*/
				echo("<div>Datai arrivati cprretamente</div>");
			}
			else
			{
				echo("Dati sbagliati");
